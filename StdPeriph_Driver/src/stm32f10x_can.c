...
void CAN_Receive(CAN_TypeDef* CANx, uint8_t FIFONumber, CanRxMsg* RxMessage) {
	...
	RxMessage->FMI = (uint8_t) 0xFF
			& (CANx->sFIFOMailBox[FIFONumber].RDTR >> 8);
	/* Get the data field */
	RxMessage->Data[0] = (uint8_t) 0xFF & CANx->sFIFOMailBox[FIFONumber].RDLR;
	RxMessage->Data[1] = (uint8_t) 0xFF
			& (CANx->sFIFOMailBox[FIFONumber].RDLR >> 8);
	...
	RxMessage->Data[5] = (uint8_t) 0xFF
			& (CANx->sFIFOMailBox[FIFONumber].RDHR >> 8);
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef* CANx, uint8_t CAN_OperatingMode) {
	...
	if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
		/* Request initialisation */
		CANx->MCR = (uint32_t) ((CANx->MCR
				& (uint32_t) (~(uint32_t) CAN_MCR_SLEEP)) | CAN_MCR_INRQ);
...
uint8_t CAN_Sleep(CAN_TypeDef* CANx) {
	...
	/* Request Sleep mode */
	CANx->MCR = (((CANx->MCR) & (uint32_t) (~(uint32_t) CAN_MCR_INRQ))
			| CAN_MCR_SLEEP);
